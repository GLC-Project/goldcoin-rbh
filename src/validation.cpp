...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx,
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool fLimitFree)
{
...

    {
        ...
        else if (!bypass_limits && gArgs.GetBoolArg("-relaypriority", DEFAULT_RELAYPRIORITY) && nModifiedFees < ::minRelayTxFee.GetFee(nSize) && !AllowFree(entry.GetPriority(chainActive.Height() + 1))) {
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool fLimitFree)
{
    std::vector<COutPoint> coins_to_uncache;
    bool res = AcceptToMemoryPoolWorker(chainparams, pool, state, tx, pfMissingInputs, nAcceptTime, plTxnReplaced, bypass_limits, nAbsurdFee, coins_to_uncache, test_accept, fLimitFree);
    if (!res) {
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool fLimitFree)
{
    ...
    return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, pfMissingInputs, GetTime(), plTxnReplaced, bypass_limits, nAbsurdFee, test_accept, fLimitFree);
}
...
bool ReadRawBlockFromDisk(std::vector<uint8_t>& block, const CDiskBlockPos& pos, const CMessageHeader::MessageStartChars& message_start, const CMessageHeader::MessageStartChars& message_start_btc)
{
    ...
    try {
        ...
        if (memcmp(blk_start, message_start, CMessageHeader::MESSAGE_START_SIZE)) {
            if (memcmp(blk_start, message_start_btc, CMessageHeader::MESSAGE_START_SIZE)) {
...
bool ReadRawBlockFromDisk(std::vector<uint8_t>& block, const CBlockIndex* pindex, const CMessageHeader::MessageStartChars& message_start, const CMessageHeader::MessageStartChars& message_start_btc)
{
    ...
    return ReadRawBlockFromDisk(block, block_pos, message_start, message_start_btc);
}
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
        // Reset subsidy height to height the final height of the GLD UTXO set
        nHeight -= consensusParams.goldcoinRBH;
        nHeight += consensusParams.goldcoinUTXOHeight;

        if (nHeight <= 26325000)
            nSubsidy = (int64_t)(50.0 / (1.1 + 0.49 * ((nHeight + 4839000) / 262800))) * COIN;
        else
            nSubsidy = 0;
    }
    return nSubsidy;
...
bool CScriptCheck::operator()() {
    ...
    if (!ret)
        LogPrintf("VerifyScript ret: false nIn: %d error: %d nFlags: %d ptxTo: %s\n", nIn, error, nFlags, ptxTo->GetHash().ToString());
    return ret;
}
        ...
        while (num--) {
            ...
            if (nTime + nExpiryTimeout > nNow) {
                ...
                AcceptToMemoryPoolWithTime(chainparams, mempool, state, tx, nullptr /* pfMissingInputs */, nTime,
                                           ...
                                           false /* test_accept */, true /* fLimitFree */);
                if (state.IsValid()) {
                    ++count;
                } else {
